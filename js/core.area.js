/**
作者：max
时间：2017-3-15 10:06:38
说明：360中出现null字符 最后一个select会为空(2017-3-15 10:07:14 已解决)
**/
!(function () {
    function CArea() {
        this.Items = {
    ...
    //清空select并添加option数组
    CArea.prototype.SetOptions = function (o, arr) {
        o.innerHTML = '';
        var ops = o.options,
            i = 0;
        ...
        for (; i < arr.length; i++) {
            ops.add(new Option(arr[i], arr[i]));
        }
    };
    //select选中option(传入select对象、要选中的字符串或下标数值) 主要用于初始化时通过js设置默认信息(不会触发callback事件)
    CArea.prototype.SetSelected = function (o, v) {
        var index = 0,
            i = 0;
        if (v === undefined) {
            return;
    ...
    //地址初始化
    CArea.prototype.init = function (json) {
        //默认省市区id和显示内容
        var _json = 
            {
                shengId: 's_sheng',
        ...
        s_sheng = document.getElementById(_json.shengId);
        s_shi = document.getElementById(_json.shiId);
        s_qu = document.getElementById(_json.quId);
        s_sheng.callback = _json.callback;
        s_shi.callback = _json.callback;
        s_qu.callback = _json.callback;
        if (_json.defaultSheng === '' && s_sheng.getAttribute('data-value')) {
            _json.defaultSheng = s_sheng.getAttribute('data-value');
        }
        if (_json.defaultShi === '' && s_shi.getAttribute('data-value')) {
            _json.defaultShi = s_shi.getAttribute('data-value');
        }
        if (_json.defaultQu === '' && s_qu.getAttribute('data-value')) {
            _json.defaultQu = s_qu.getAttribute('data-value');
        }
        //省第一次的信息要加载
        this.SetOptions(s_sheng, this.Items['0']);
        //事件绑定
        s_sheng.onchange = function () {
            oThis.SetOptions(s_shi, oThis.Items['0_' + (this.selectedIndex - 1)]);
            oThis.SetOptions(s_qu, oThis.Items['0_' + (s_sheng.selectedIndex - 1) + '_' + (this.selectedIndex - 1)]);
            //回调
            if (this.callback) {
                this.callback(s_sheng, s_sheng.options[this.selectedIndex]);
            }
        };
        s_shi.onchange = function () {
            oThis.SetOptions(s_qu, oThis.Items['0_' + (s_sheng.selectedIndex - 1) + '_' + (this.selectedIndex - 1)]);
            //回调
            if (this.callback) {
                this.callback(s_shi, s_shi.options[this.selectedIndex]);
            }
        };
        s_qu.onchange = function () {
            if (this.callback) {
                this.callback(s_qu, s_qu.options[this.selectedIndex]);
            }
        };
        /*
        defaultSheng: '请选择',
        ...
        */
        //处理初始设置
        if (_json.defaultSheng !== '') {
            this.SetSelected(s_sheng, _json.defaultSheng);
            oThis.SetOptions(s_shi, oThis.Items['0_' + (s_sheng.selectedIndex - 1)]);
            if (_json.defaultShi !== '') {
                this.SetSelected(s_shi, _json.defaultShi);
                oThis.SetOptions(s_qu, oThis.Items['0_' + (s_sheng.selectedIndex - 1) + '_' + (s_shi.selectedIndex - 1)]);
                if (_json.defaultQu !== '') {
                    this.SetSelected(s_qu, _json.defaultQu);
                }
            }
        }
    };
